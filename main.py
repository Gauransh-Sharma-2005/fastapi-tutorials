...
class Patient(BaseModel):
  ...
  # pydantic model will first trigger verdict and then bmi
    @computed_field
    @property
    def bmi(self) -> float:
...
# utility function which saves the data of the new patient in the existing database
def save_data(data):
...
def create_patient(patient: Patient):
  ...
  # step 2 - check if the patient already exists
  if patient.id in data:
    raise HTTPException(status_code=400, detail='Patient already exists')
  

  # step 3 - new patient add to the database
